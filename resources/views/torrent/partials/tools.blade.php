<div x-data="{'moderateTorrent': false}">
                    ...
                    <td style="display: flex; justify-content: space-around; padding: 10px 0">
                        ...
                        @if (auth()->user()->group->is_modo)
                            <div class="torrent-moderation-controls">
                                ...
                                <button class="btn btn-labeled btn-warning btn-xs @if ($torrent->isPostponed()) disabled @endif"
                                        @click="moderateTorrent = {'type': 'postpone',
                                                    'history': {{ ($torrent->history()->count() > 0 ? 'true' : 'false') }},
                                                    'id': {{$torrent->id}}, 'name': '{{$torrent->name}}' };">
                                    <i class="{{ config('other.font-awesome') }} fa-pause"></i> {{ __('common.moderation-postpone') }}
                                </button>

                                <button class="btn btn-labeled btn-danger btn-xs @if ($torrent->isRejected()) disabled @endif"
