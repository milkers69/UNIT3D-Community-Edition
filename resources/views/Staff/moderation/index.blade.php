...
@section('content')
<div>
    <div class="panelV2" x-data="{ tab: 0, reviewTorrent: false }">
        ...
        <div class="panel__body">
            <template x-if="( tab === 1 || tab === 0 ) && !reviewTorrent">
                <div class="block" style="margin: 0 !important;
                        border-radius: 0 !important;
                        border: none !important;
                        box-shadow: none !important;">
                    <div><p class="h3">Pending Torrents</p></div>
                    <hr />
                        ...
                        <table class="table table-condensed table-striped table-bordered">
                            ...
                            @foreach($pending as $p)
                                <tr>
                                    ...
                                    <td class="torrents-filename"><a target="_blank" href="{{route('torrent', ['id'=> $p->id])}}">{{$p->name}}
                                    ...
                                    <td>
                                        <button class="btn btn-labeled btn-info btn-xs" @click="reviewTorrent = '{{ route('torrent', ['id' => $p->id]) }}?iframe=1'; tab = 4;"><i class="fas fa-file-search"></i> Review</button>
                            ...
                            @foreach($postponed as $p)
                                <tr>
                                    <td class="text-center">@if( $p->updated_at->greaterThan($p->moderated_at))
                                        ...
                                        @endif</td>
                                    <td style="font-size: 1.2rem">{{$p->moderated_at->diffForHumans()}}<br><span style="font-size: 1rem">{{\App\Models\User::find($p->moderated_by)->username}}</span></td>
                                    <td class="torrents-filename"><a target="_blank" href="{{route('torrent', ['id'=> $p->id])}}">{{$p->name}}
                                            <i style="font-size: 1rem" class="fas fa-external-link"></i></a></td>
                                    <td>{{$p->category->name}}</td>
                                    <td>{{$p->type->name}}</td>
            ...
            <template x-if="( tab === 3 || tab === 0 ) && !reviewTorrent">
                <div class="block" style="
                                        margin: 0 !important;
                                        border-radius: 0 !important;
                                        border: none !important;
                                        box-shadow: none !important;">
                    <div><p class="h3">Rejected Torrents</p></div>
                    <hr />
                    <div class="table-responsive">
                        <table class="table table-condensed table-striped table-bordered">
                            <thead>
